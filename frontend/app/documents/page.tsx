...
export default function Documents() {
  const [docs, setDocs] = useState<Doc[]>([]); const [selected, setSelected] = useState<string[]>([]); const [message, setMessage] = useState(''); const [bulk, setBulk] = useState<BulkItem[]>([]); const [pendingDuplicate, setPendingDuplicate] = useState<File|null>(null); const [ready, setReady] = useState(false); const [query, setQuery] = useState(''); const [family, setFamily] = useState(''); const [fieldValue, setFieldValue] = useState(''); const [uploadedFrom, setUploadedFrom] = useState(''); const [uploadedTo, setUploadedTo] = useState(''); const [page, setPage] = useState(1); const [total, setTotal] = useState(0);
  const token = () => localStorage.getItem('access_token') ?? '';
  async function load(nextPage = page) {
    const accessToken = token();
    if (!accessToken) { window.location.href = '/login'; return; }
    const params = new URLSearchParams({page:String(nextPage), page_size:'10'}); if (query) params.set('q', query); if (family) params.set('family', family); if (fieldValue) params.set('field_value', fieldValue); if (uploadedFrom) params.set('uploaded_from', `${uploadedFrom}T00:00:00Z`); if (uploadedTo) params.set('uploaded_to', `${uploadedTo}T23:59:59Z`);
