...
export default function Documents() {
  const [docs, setDocs] = useState<Doc[]>([]); const [message, setMessage] = useState(''); const [bulk, setBulk] = useState<BulkItem[]>([]); const [pendingDuplicate, setPendingDuplicate] = useState<File|null>(null);
  ...
  return <><h1>Documents</h1><button onClick={logout}>Logout</button> <Link href="/security">Security</Link>
    <h2>Single upload</h2><form onSubmit={upload}><label htmlFor="single-file">Choose PDF, JPG or PNG</label><input id="single-file" name="file" type="file" accept="application/pdf,image/jpeg,image/png" required/><button>Upload</button></form>
    ...
    <h2>Your documents</h2>{docs.map(d=><div className="card" data-testid="document-card" key={d.id}><Link href={`/documents/${d.id}`}><strong>{d.original_filename}</strong></Link><div className="muted">{d.status} · {d.size_bytes} bytes{d.duplicate_of_document_id ? ' · kept duplicate' : ''}</div></div>)}</>;
}
