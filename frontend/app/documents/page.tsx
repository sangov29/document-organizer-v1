...
type Doc = {id:string; original_filename:string; status:string; size_bytes:number; uploaded_at:string; duplicate_of_document_id?:string|null; tags:Named[]; collections:Named[]};
...
export default function Documents() {
  const [docs, setDocs] = useState<Doc[]>([]); const [selected, setSelected] = useState<string[]>([]); const [message, setMessage] = useState(''); const [bulk, setBulk] = useState<BulkItem[]>([]); const [pendingDuplicate, setPendingDuplicate] = useState<File|null>(null); const [ready, setReady] = useState(false); const [query, setQuery] = useState(''); const [family, setFamily] = useState(''); const [fieldValue, setFieldValue] = useState(''); const [uploadedFrom, setUploadedFrom] = useState(''); const [uploadedTo, setUploadedTo] = useState(''); const [page, setPage] = useState(1); const [total, setTotal] = useState(0); const [tags, setTags] = useState<Named[]>([]); const [collections, setCollections] = useState<Named[]>([]); const [tagFilter, setTagFilter] = useState(''); const [collectionFilter, setCollectionFilter] = useState(''); const [reminders, setReminders] = useState<Reminder[]>([]);
  ...
  async function loadReminders() {
    const response = await fetch(`${API}/api/v1/documents/reminders?within_days=90`, {headers:{Authorization:`Bearer ${token()}`}});
