...
Verified commit: `502b9836e5eb8c1260a12d75cc5fdb6449ab3f0c`  
Evidence: GitHub Runtime Acceptance #25  
GitHub Actions run ID: `35965339518`  
Evidence URL: https://github.com/sangov29/document-organizer-v1/actions/runs/35965339518
...
- Backend source/model contracts: **93 passed**, with separate mandatory JUnit evidence
...
Runtime Acceptance #25 confirms dependency-aware process liveness and traffic readiness for PostgreSQL, Redis and object storage. It also proves isolated PostgreSQL dump/restore equivalence, MinIO object round-trip restore, the measurable acceptance SLO gate and controlled authenticated list/search load. The larger timing sample preserves the existing anti-enumeration tolerances while reducing false failures at a discrete KS boundary.
...
Runtime Acceptance #25 proves the deterministic V1 behavior at commit `502b983` against the Docker acceptance stack: PostgreSQL, Redis, MinIO, Mailpit, API, Celery worker, frontend and Playwright. It also proves isolated database and object-storage restoration, the synthetic acceptance SLO gate and the bounded authenticated read-load profile within the acceptance environment. The workflow counter restarted because `.github/workflows/blank.yml` was renamed to `runtime-acceptance.yml`. Mail evidence is limited to application queueing, token lifecycle, SMTP handoff and local Mailpit receipt; it is not an external-provider delivery claim.
...
With backup/restore, measurable SLOs and controlled read-load qualification now verified, the next unblocked work is rate-limit and security validation, followed by broader write/OCR load and resilience testing. PP-StructureV3 remains out of scope until the private corpus gate is satisfied.
